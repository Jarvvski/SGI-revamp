	...
	SGI_Invites:SetScript("OnUpdate",function()
		...
		for i = 1,30 do
			if not SGI_Invites.items[i] then
				...
				SGI_Invites.items[i].highlight:SetColorTexture(1,1,0,0.2)
...
function SGI:CreateSmallSuperScanFrame()
	...
	SuperScanFrame.progressTexture:SetColorTexture(1,0.5,0,0.4);
			...
			for k,_ in pairs(F) do
				if (not SGI_FilterHandle.filterFrames[k]) then
					...
					if (SGI_DATA[SGI_DATA_INDEX].settings.filters[k].active) then
						SGI_FilterHandle.filterFrames[k].highlight:SetColorTexture(0,1,0,0.2);
					else
						SGI_FilterHandle.filterFrames[k].highlight:SetColorTexture(1,0,0,0.2);
					end
					SGI_FilterHandle.filterFrames[k]:SetScript("OnEnter", function(self)
						self.highlight:SetColorTexture(1,1,0,0.2);
...
local function CreateOptions()
	...
	SGI_Options.optionHelpText:SetPoint("TOP",SGI_Options,"TOP",40,-40);
	...
	SGI_Options.limitLow:SetWidth(45)
	SGI_Options.limitLow:SetHeight(40)
	SGI_Options.limitLow:SetPoint("CENTER",SGI_Options,"CENTER",15,80)
	...
	SGI_Options.limitLow.texture:SetColorTexture(1,1,0,0.2)
	...
	SGI_Options.limitHigh.texture:SetColorTexture(1,1,0,0.2)
	...
	SGI_Options.raceLimitHigh.texture:SetColorTexture(1,1,0,0.2)
	...
	SGI_Options.classLimitHigh.texture:SetColorTexture(1,1,0,0.2)
	...
	SGI_Options.Interval.texture:SetColorTexture(1,1,0,0.2)
